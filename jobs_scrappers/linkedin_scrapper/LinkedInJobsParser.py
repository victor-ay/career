...
class LinkedInJobsParser():
    ...
    def _put_description_field_or_none(self, job_id:str, key: str, value , default = None):
        try:
            ...
            print(f" Excepted at [ {e} ] -> job_id = {job_id} ; key= {key} ; value = {value}")
                    ...
                    try:
                        ...
                        if "navigationBarSubtitle" in keys:
                            ...
                            self._put_description_field_or_none(
                                ...
                                value = short_include.get('companyLogo').get('actionTarget')
                            )
                            ...
                            for i in range(len(short_include['primaryDescription']['attributesV2'])):
                                if short_include['primaryDescription']['attributesV2'][i]['detailDataUnion'].get('epoch'):
                                    epochAt = short_include['primaryDescription']['attributesV2'][i]['detailDataUnion'].get('epoch').get('epochAt')
                                    break
                                else:
                                    epochAt = None
                            self._put_description_field_or_none(
                        ...
                        if keys == "hasConfirmedEmailAddress":
                            if short_include.get('companyApplyUrl'):
                                companyApplyUrl = short_include.get('companyApplyUrl')
                            else:
                                companyApplyUrl = 'https://www.linkedin.com/jobs/view/' + str(job_num)+'/'
                        ...
                        if keys in ["*jobDetailsSummaryCard"] \
                                and short_include.get('description'):
                        ...
                        if keys in ["descriptionText"] and short_include.get('description'):
                            text_init = short_include['description']['text']
                            text_format = short_include['description']['attributesV2']
                            # self._jobs_obj["job_description"] = self._format_LinkedIn_job_description(format_list=text_format,
                            #                                                              description=text_init)
                            self._put_description_field_or_none(
                                job_id=job_num,
                                key="job_description",
                                value=self._format_LinkedIn_job_description(format_list=text_format,description=text_init)
                            )
                        if keys in ["postedOnText", "descriptionText"]:
                            if short_include["descriptionText"].get('text'):
                                text_init = short_include["descriptionText"].get('text')
                                text_format = short_include["descriptionText"].get('attributesV2')
...
if __name__ == '__main__':
    with open('scraped_files/israel_test_4_1-2023-05-03.json', 'r') as fh:
